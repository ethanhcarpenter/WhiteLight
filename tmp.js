...
const albums = [
    ...
    {
        key: 'birthdayblizzard',
        title: 'Birthday Blizzard 26',
        year: '2026 · Mixtape',
        color: '#a8c0d8',
        tracks: [
            'BRONX ZOO FREESTYLE',
...
let audio = window.parent.getAudio(); // Get the shared audio element from the parent window
let isPlaying = false;
...
function togglePlay() {
  ...
  if (isPlaying) {
    audio.pause();
    document.getElementById('platterLabel').style.animationPlayState = 'paused';
...
function moveTrack(amount) {
  const album = albums.find(a => a.key === selectedKey);
  const fileName = decodeURIComponent(audio.src.split('/').pop());
  const currentTrack = fileName.replace('.mp3', '').replace('J. Cole - ', '').trim();
  const currentIndex = album.tracks.findIndex(t => t.trim() === currentTrack);
  let nextIndex = (currentIndex + amount) % album.tracks.length;
...
function buildShelf() {
  ...
  albums.forEach((album, i) => {
    ...
    slotData.push({el: slot, cover, img, left, zIdx, album});
  });
...
function getPlatterCenter() {
  const outer = document.getElementById('platterOuter');
...
function pickAlbum(key, slot, cover, img) {
  ...
  if (selectedKey) returnCard(false);
  selectedKey = key;
  ...
  requestAnimationFrame(() => {
    ...
    flying.style.cssText = `left:${slotRect.left}px;top:${slotRect.top}px;width:${CARD_W}px;height:${CARD_W}px;transition:none;`;
...
function setPlatter(cover, img) {
  const platter = document.getElementById('platterLabel');
...
function returnCard(reset = true) {
  ...
  if (!key) return;
  if (reset) selectedKey = null;

  const sd = slotData.find(s => s.album.key === key);
  if (!sd) return;
